...
public class SuperMetaNode : SuperContainer 
{
	...
	public void RemoveAndCacheAllChildren()
	{
		...
		{
			Debug.Log("[WARNING] UNABLE TO USE EDITOR CACHE FOR DUPLICATE KEYS");
	...
	public void ProcessMetadata()
	{
        ...
        if(json.ContainsKey("children"))
        {
            if(rootContainer == "(root)")
            {
                ...
                {
                    rootWidth = Convert.ToSingle(json["root_width"]);
                    rootHeight = Convert.ToSingle(json["root_height"]);
                    
                    GetComponent<RectTransform>().sizeDelta = new Vector2(rootWidth, rootHeight);
                }
                ...
                for(int i = 0; i < children.Count; i++)
                {
                    ...
                    if(node_type == "container" && node_name == rootContainer)
                    {
                        ...
                        List<object> size = raw_node["size"] as List<object>;
                        rootWidth = Convert.ToSingle(size[0]);
                        rootHeight = Convert.ToSingle(size[1]);
                        GetComponent<RectTransform>().sizeDelta = new Vector2(rootWidth, rootHeight);
